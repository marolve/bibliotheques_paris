...
function callParisfr(callback) {
	https.get('https://www.paris.fr/lieux/bibliotheques/tous-les-horaires', (resp) => {
		let data = '';
...
// return array
// {
//    name: 'Bibliothèque Parmentier',
//    arrondissement: '11ᵉ',
//    link: 'https://www.paris.fr/lieux/bibliotheque-parmentier-1723',
//    schedules: [
//      [ {starthour: 10, startminute: 00, endhour: 13, endminute : 00 },
//	      {starthour: 14, startminute: 00, endhour: 18, endminute : 00 }
//      ],
...
function analyzeParisfr(html) {
	...
	// extract bibliotheque and schedules
	var bibliotheques = [];
	...
	tableitems.each((i, tableitem) => {
		...
		divs.each((j, div) => {
			...
			if (j == 0) {
				...
				if (pos != -1 && pos > 1) {
					bibliotheque.arrondissement = text.substring(pos - 2, pos + 1);
				} else if (text.indexOf('Paris centre') != -1) {
					bibliotheque.arrondissement = 'Paris centre';
				}
				let posstart = text.indexOf('href="');
				let posend = text.indexOf('">');
				if (posstart != -1 && posend != -1 && posstart < posend) {
					bibliotheque.link = 'https://www.paris.fr' + text.substring(posstart + 6, posend);
				}
			} else {
				...
				bibliotheque.schedules.push(schedules);
			}
		});
	});
...
function loadHtml(callback) {
	fs.readFile('bibliotheques.html', (err, data) => {
		if (!err) {
...
// Fill Html with bibliotheques Data
	...
	for (const bibliotheque of data.bibliotheques) {
		if (bibliotheque.schedules.length > 7) {
			...
			for (let i = 0; i < 10; i++) {
				let scheduleLabel = '';
				let scheduleData = '';
				for (const schedule of bibliotheque.schedules[i]) {
			...
			bibliothequeRows += row.replace('%arr%', bibliotheque.arrondissement).replace(/%name%/g, bibliotheque.name).replace(/%link%/g, bibliotheque.link).replace('%schedulelabels%', scheduleLabels).replace('%scheduledatas%', scheduleDatas);
		}
	};
	html = html.replace('%rows%', bibliothequeRows);
...
console.log('listening at port 5045...');